...
# echo PBF="/user/local/bin/$(echo $REGION | grep -o '[^/]*$')-latest.osm.pbf"


PBF="/data/british-columbia-latest.osm.pbf"

# Check if the PBF file exists
if [ -f "$PBF" ]; then
    echo "Importing PBF file: $PBF"
else
    echo "PBF file not found at: $PBF"
    exit 1
fi
